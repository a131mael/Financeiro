package org.aaf.financeiro.util.constantes;

public class CNB240_SICOOB_CONSTANTS_TEFAMEL{

	//TODO CAMPO 1
	public static final String COD_BANCO = "756"; //CODIGO DA SICOOB
	public static final String COD_MOEDA = "9"; //MOEDA = REAL
	public static final String COD_CARTEIRA = "1"; // codigo da  Carteira
	public static final String COD_AGENCIA = "3069"; // codigo da agencia do beneficiario
	public static final String VER_CAMPO1 = "8" ; //CODIGO VERIFICADOR DO CAMPO 1
	
	//TODO CAMPO 2
	public static final String COD_MODALIDADE = "01"; //Simples com Registro
	public static final String COD_BENEFICIARIO = "1199870"; //Codigo do beneficiario no banco
	public static final String NOSSO_NUMERO = "10000"; //Nosso NUmero de boleto começa em 10000 e eh unico para cada boleto
	public static final String VER_CAMPO2 = "2" ; //CODIGO VERIFICADOR DO CAMPO 2
	
	
	//TODO CAMPO 3
	public static final String NUM_PARCELA = "001"; //NUMERO DA PARCELA
	public static final String VER_CAMPO3 = "7" ; //CODIGO VERIFICADOR DO CAMPO 2
	
	//TODO CAMPO L
	public static final String VER_BOLETO = "8" ; //CODIGO VERIFICADOR DO CAMPO 2
	
	
	//TODO CAMPO 4
	public static final String FATOR_VENCIMENTO = "001"; //NUMERO DA PARCELA
	public static final String VALOR_BOLETO = "8" ; //CODIGO VERIFICADOR DO CAMPO 2
		
	
	public static final String COD_COOPERATiVA = "4434"; //CODIGO DA COOPERATIVA PARA DV do nosso numero
	
	public static final String CNPJ = "03.660.921/0001-79"; //CNPJ da Tefamel
	public static final String NOME = "FAVO DE MEL TRANSPORTE ESCOLAR E TURISMO LTDA - M"; //CNPJ da Tefamel
	public static final String RUA = "Rua José Cosme Pamplona"; //CNPJ da Tefamel
	public static final String BAIRRO = "Bela Vista"; //CNPJ da Tefamel
	public static final String CIDADE = "PALHOCA"; //CNPJ da Tefamel
	public static final String CEP = "88132-700"; //CNPJ da Tefamel
	public static final String UF = "SC"; //CNPJ da Tefamel
	
	public static final String VALORJUROSAODIA = "050";//50 centavos
	public static final String VALORDESCONTO = "2000";//30 reais
	public static final String VALORMULTA = "200";//2 % de multa
}
